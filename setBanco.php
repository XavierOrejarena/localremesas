...
if ($id_banco == 5) {
    ...
    } elseif ($id_pago_in = mysqli_fetch_assoc(mysqli_query($link, "SELECT id FROM pagos_in WHERE RIGHT(referencia) = RIGHT('$referencia') AND monto = '$monto' AND id_banco = '$id_banco' AND estado = 'APROBADO'"))['id']) {
        $res['errores'][] = true;
        $res['mensajes'][] = 'Este pago ya fue aprobado y agregado.';
    } elseif (mysqli_num_rows(mysqli_query($link, "SELECT id FROM pagos_in WHERE RIGHT(referencia) = RIGHT('$referencia') AND monto = '$monto' AND id_banco = '$id_banco' AND flag = 1")) > 0) {
            $res['errores'][] = false;

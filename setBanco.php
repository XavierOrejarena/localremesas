...
if ($id_banco == 5) {
    if ($id_pago_in = mysqli_fetch_assoc(mysqli_query($link, "SELECT id FROM pagos_in WHERE RIGHT(referencia, 6) = RIGHT('$referencia', 6) AND monto = '$monto' AND id_banco = '$id_banco' AND estado = 'PENDIENTE' AND flag IS NULL"))['id']) {
        if (mysqli_query($link, "UPDATE bancos SET saldo = saldo + '$monto' WHERE id = '$id_banco'")) {
            $res['errores'][] = false;
            $res['mensajes'][] = "Saldo actualizado existosamente.";
            if (mysqli_query($link, "UPDATE pagos_in SET flag = 1, estado = 'APROBADO' WHERE id = '$id_pago_in'")) {
                $res['errores'][] = false;
    ...
    }elseif ($id_pago_in = mysqli_fetch_assoc(mysqli_query($link, "SELECT id FROM pagos_in WHERE RIGHT(referencia, 6) = RIGHT('$referencia', 6) AND monto = '$monto' AND id_banco = '$id_banco' AND flag IS NOT NULL"))['id']) {
        $res['errores'][] = true;
    ...
    }elseif (!($id_pago_in = mysqli_fetch_assoc(mysqli_query($link, "SELECT id FROM pagos_in WHERE RIGHT(referencia, 6) = RIGHT('$referencia', 6) AND id_banco = '$id_banco' AND estado = 'PENDIENTE' AND flag IS NULL"))['id'])) {
        $res['errores'][] = false;
        $res['mensajes'][] = "XD";
    }
}
    ...
    //     $res['errores'][] = false;
    //     $res['mensajes'][] = 'Pago agregado existosamente3.';
    // } else {
    //     $res['errores'][] = true;
    ...
    //         mysqli_query($link, "UPDATE bancos SET saldo = saldo + '$monto' WHERE id = '$id_banco'");
    //         $res['errores'][] = false;
    //         $res['mensajes'][] = 'Pago agregado existosamente.3';
    //     } else {
    //         $res['errores'][] = true;
    //         $res['mensajes'][] = 'Hubo un error agregando el pago.';
    //     }
